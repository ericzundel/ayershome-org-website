...
init_preamble() {
    ...
    if [ -d "${WORKSPACE_ROOT}/.venv/bin/" ]; then
        ...
        echo "Warning: virtual environment named '.venv' not found at ${WORKSPACE_ROOT}/.venv"
        echo "  You can create it by running 'uv venv --python \${PYTHON_VERSION}'"
    fi
}
...
# run_python_tool <module_path> [args...]
#
# Run a Python tool from the tools/ directory using uv.
# The module_path should be relative to the tools/ directory.
#
# Example:
#   run_python_tool validate_config --help
#   run_python_tool utils/check_dns example.com
#
run_python_tool() {
    local module_name=$1
    shift

    # Ensure we're in the workspace root for uv to find pyproject.toml
    cd "${WORKSPACE_ROOT}"

    # Run with uv to ensure correct Python version and dependencies
    uv run python -m "tools.${module_name}" "$@"
}
